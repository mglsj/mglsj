...
export async function fetchGitHubContributions(): Promise<GitHubContributions> {
	...
	return parseContributions(
		mergeContributions(elevenMonthsData, twelfthMonthData),
	);
}
...
export async function fetchGitHubLanguages(): Promise<GitHubLanguages> {
	...
	if (!response.ok) {
		throw new Error(
			`${response.status}: ${response.statusText} while fetching GitHub user contributions.`,
		);
	}
...
function mergeContributions(
	dataA: ContributionsCollection,
	dataB: ContributionsCollection,
) {
...
function parseLanguages(data: ContributionsCollection): GitHubLanguages {
	...
	for (const contributions of data.user.contributionsCollection
		...
		for (const languageEdge of contributions.repository.languages.edges) {
			...
			allLanguages[languageEdge.node.name] =
				(allLanguages[languageEdge.node.name] ?? 0) + languageEdge.size;
...
function parseContributions(
	...
	for (const contributionWeek of data.user.contributionsCollection
		...
		for (const contributionDay of contributionWeek.contributionDays) {
			...
			if (
				...
				(lastContribution && lastContribution.date.getTime() !== date.getTime())
			) {
